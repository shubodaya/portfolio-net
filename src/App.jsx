...
const projects = [
  {
    ...
    year: '2025',
    detail:
      'Built scalable labs for deployment and troubleshooting across Cisco routers, switches, and enterprise firewalls.',
    skills: ['Cisco IOS', 'EVE-NG', 'Network automation'],
  },
  {
    ...
    skills: ['Windows Server', 'Linux', 'Active Directory'],
  },
  {
    ...
    skills: ['Azure VNets', 'Sentinel', 'Cloud routing'],
  },
  {
  ...
  },
  {
    ...
    skills: ['OWASP Top 10', 'WAF tuning', 'Application Security'],
  },
  {
    ...
    skills: ['ISO 21434', 'Software-defined firewall', 'TARA'],
  },
]
...
const staticNocLogs = [
  { level: 'debug', time: '19:27:48', message: 'SNMP poll jitter normal node=SW-03' },
  { level: 'debug', time: '19:27:50', message: 'Routing table refresh completed in 42ms' },
  { level: 'notice', time: '19:27:51', message: 'ISP failover ready path=Primary' },
  { level: 'debug', time: '19:27:53', message: 'NetFlow sample exported collector=10.50.1.20' },
  { level: 'notice', time: '19:27:55', message: 'ISP failover ready path=Primary' },
  { level: 'error', time: '19:27:56', message: 'WAN circuit down site=Bristol' },
  { level: 'notice', time: '19:27:58', message: 'SNMP trap cleared node=FW-02' },
  { level: 'warn', time: '19:27:59', message: 'Packet loss 2% site=Manchester' },
]
...
const interestHighlights = [
  {
    title: 'Creative',
    icon: '🎨',
    detail: 'Art, 3D modeling, film/tactile editing, and music composition.',
    note: 'Storytelling + visuals keep operations presentations engaging.',
    direction: -1,
  },
  {
    title: 'Active',
    icon: '🏏',
    detail: 'Cricket tournaments, gym blocks, and competitive gaming.',
    note: 'Positive energy + strategy fuel long shifts and quick escalations.',
    direction: 1,
  },
  {
    title: 'Languages',
    icon: '🗣️',
    detail: 'English, Hindi, Kannada, Telugu, Tamil.',
    note: 'Multilingual comms keep documentation and handovers clear globally.',
    direction: -1,
  },
]
...
export default function App() {
  ...
  const [nocLogs, setNocLogs] = useState(staticNocLogs)
  ...
  const nocShiftIntervalRef = useRef(null)
  const nocShiftTimeoutRef = useRef(null)
  ...
  useEffect(() => {
    ...
    shiftOnce()
    nocShiftIntervalRef.current = window.setInterval(shiftOnce, 2200)

    return () => {
      if (nocShiftIntervalRef.current) window.clearInterval(nocShiftIntervalRef.current)
      if (nocShiftTimeoutRef.current) window.clearTimeout(nocShiftTimeoutRef.current)
    }
  }, [])
  ...
  const interestVariants = {
    hidden: (direction) => ({ opacity: 0, x: direction * 80 }),
    show: (direction) => ({
      opacity: 1,
      x: 0,
  ...
  const skillVariants = {
    hidden: (direction) => ({ opacity: 0, x: direction * 60 }),
    show: (direction) => ({
      opacity: 1,
      x: 0,
      ...
      <main>
        ...
        >
          <div className="hero__pin">
            ...
            <div className="noc-panel">
              ...
              >
                <div className={`noc-panel__stream ${isNocShifting ? 'noc-panel__stream--shift' : ''}`}>
        ...
        <section className="section" id="interests">
          <div className="section__header" data-reveal>
            ...
            <p className="section__note">
              Passions that keep the curiosity sharp and the problem-solving mindset fresh.
            </p>
          </div>
          <div className="interest-grid">
            {interestHighlights.map((interest, idx) => (
              <motion.article
                key={interest.title}
                className="interest-card"
                variants={interestVariants}
                custom={interest.direction}
                initial="hidden"
                whileInView="show"
                viewport={{ once: true, amount: 0.4 }}
              >
                <div className="interest-card__icon" aria-hidden="true">
                  {interest.icon}
                </div>
                <div className="interest-card__body">
                  <h3>{interest.title}</h3>
                  <p>{interest.detail}</p>
                  <p className="interest-card__note">{interest.note}</p>
                </div>
              </motion.article>
            ))}
          </div>
